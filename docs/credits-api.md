...
`MAX_CONCURRENT_PER_KEY` 默认 4，必须为正整数；仅限制同时在途请求，不预占金额。已配置文本价格的模型默认输出上限 4096、最大上限 8192，可在价格中覆盖；客户端提交超出最大值的输出上限返回 400，不静默缩短。图片 `n` 为 1–10。超额没有固定金额保证，取决于单次费用与并发数。
...
公共 `/v1/...` 成功响应协议保持兼容。总额度耗尽、窗口耗尽、并发上限为 429；窗口耗尽含 `Retry-After`，总额度耗尽不提供重置时间。价格缺失沿用已有 429 错误；配置全零非 free 为 503；图片规则或输出上限不合法为 400。不要依赖错误文案解析余额，应查询上述元数据接口。
