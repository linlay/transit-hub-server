...
func TestListAPIKeysIncludesWindowUsage(t *testing.T) {
	...
	key, err := db.CreateAPIKey(t.Context(), store.CreateAPIKeyParams{Name: "windowed", RateLimits: []store.RateLimit{
		{Window: "5h", RequestQuota: 10, TokenQuota: 100, CostQuotaMicro: 1000},
		{Window: "7d", RequestQuota: 50, TokenQuota: 1000, CostQuotaMicro: 5000},
	}})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, item := range result.Items {
		...
		if item.UsedRequests != 2 || item.UsedTokens != 307 || item.UsedCostMicro != 9250 {
		...
		for i, status := range item.RateLimitUsage {
			if status.Window != []string{"5h", "7d"}[i] || status.Requests != 1 || status.Tokens != 7 || status.CostMicro != 250 {
				t.Fatalf("window: %#v", status)
			}
			if !status.ResetsAt.After(now) || status.StartsAt.After(now) {
				t.Fatalf("bounds: %#v", status)
			}
			if status.RequestRemaining != status.RequestQuota-1 || status.TokenRemaining != status.TokenQuota-7 || status.CostRemainingMicro != status.CostQuotaMicro-250 {
