...
func TestDeviceKeyBindingValidationRecoveryAndIsolation(t *testing.T) {
	...
	svc.Config.QuotaMicrocredits = 1_000_000
	svc.Config.RateLimits = []accesskey.RateLimit{{Window: "1h", QuotaMicrocredits: 100_000}}
	...
	if first.Source != "access_token" || first.IssuerJTI != "" || first.RequestQuota != 50 || first.QuotaMicrocredits != 1_000_000 || len(first.RateLimits) != 1 || first.RateLimits[0].QuotaMicrocredits != 100_000 {
