...
func TestRateLimitStatusesIgnoreLogsOutsideFixedWindow(t *testing.T) {
	...
	created, err := control.CreateAPIKey(t.Context(), CreateAPIKeyParams{
		Name:       "windowed",
		RateLimits: []RateLimit{{Window: RateLimitWindow1H, RequestQuota: 10, TokenQuota: 100, QuotaMicrocredits: 1000}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	status := statuses[0]
	if status.Requests != 1 || status.Tokens != 7 || status.ChargedMicrocredits != 11 {
		t.Fatalf("unexpected status usage: %#v", status)
	}
	if status.RequestRemaining != 9 || status.TokenRemaining != 93 || status.RemainingMicrocredits != 989 {
