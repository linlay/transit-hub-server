...
Docker 构建默认沿用 Go 官方镜像的模块代理配置。仅在需要国内代理的主机上通过构建参数覆盖，例如：

```bash
# ssh singapore02 并进入部署目录后：使用默认配置
docker compose up -d --build

# ssh sas01 并进入部署目录后：仅本次构建使用 goproxy.cn
BUILD_GOPROXY=https://goproxy.cn,direct docker compose up -d --build
```
...
`GET /admin/traffic/analytics` 提供按时间、API Key、模型、Provider 和请求结果筛选的汇总、趋势及排行；`GET /admin/logs` 支持同一组筛选并分页。参数、时区、失败口径和验证方式见 [Traffic 分析接口](docs/traffic-analytics.md)。
